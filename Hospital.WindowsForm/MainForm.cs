...
namespace Hospital.WindowsForm
{
    public partial class MainForm : Form
    {
        ...
        private void startQuestionStrictMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                //  Відкривається форма для входу в профіль
                AuthForm auth = new AuthForm(_context);
                auth.ShowDialog();
            }
        }
        private void showSessionsResults_Click(object sender, EventArgs e)
        {
            //  Перевірка чи користувач аторизувався
            if (UserAccount.user != null)
            {

                this.Visible = false;
                //  Формування запиту, який дістає сесії з БД, що належать користувачеві
                var item = _context.Users.Where(x => x.Id == UserAccount.user.Id).SelectMany(x => x.Sessions).AsQueryable();

                if (item != null)
                {
                    ResultTestForm form = new ResultTestForm(item);
                    form.ShowDialog();
                }
